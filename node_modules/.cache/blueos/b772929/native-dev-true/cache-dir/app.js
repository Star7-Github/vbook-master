...
async function downloadLoop(book) {
  ...
  if (fileList.length > 0) {
    ...
    if (fileList.length <= 100) {
      prompt.showToast({
        message: `首文件: ${fname} (总${fileList.length})`
      });
    }
  } else {
    prompt.showToast({
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (consecutiveFailCount > 50) {
      prompt.showToast({
        message: `${book.name} 失败过多，请稍后重试以补全`
      });
      task.status = "error";
      break;
    }
    ...
    if (existingSet.has(chapterFileName)) {
      ...
      try {
        ...
        if (text) {
          ...
          totalFailCount++;
          console.log(`[App] Download fail ch ${i}, consecutive: ${consecutiveFailCount}`);
        }
      } catch (e) {
        consecutiveFailCount++;
        totalFailCount++;
